 ...
 * of python-ecjson has been preserved, so without this keyword argument
...
decode_string(DecoderContext *ctx)
{
    PyObject *object;
    register char *ptr;
    register char c, escaping;
    char string_escape, has_escaped_slash, has_unicode;
    int len;
    char *start;
    char *copied_string;
    char *p;
    ...
    /* look for the closing quote, detect escaping and unicode */
    escaping = False;
    has_escaped_slash = False;
    string_escape = False;
    ...
    if(has_escaped_slash) {
        copied_string = malloc(len);
        if(copied_string == NULL) {
            PyErr_SetString(PyExc_MemoryError, "error allocating memory while decoding JSON");
            return NULL;
        }
        q = copied_string;
        for(p=start; p<ptr;) {
    ...
    if (has_unicode)
        object = PyUnicode_DecodeUnicodeEscape(start, len, NULL);
    else if (string_escape)
        object = PyString_DecodeEscape(start, len, NULL, 0, NULL);
    else
        object = PyString_FromStringAndSize(start, len);
...
encode_object(EncoderContext *ctx, PyObject *object)
{
    ...
    } else if (PyObject_HasAttrString(object,"_as_javascript")) {
        return PyObject_Call(PyObject_GetAttrString(object,"_as_javascript"),PyTuple_New(0),NULL);
...
static PyMethodDef ecjson_methods[] = {
...
initecjson(void)
{
    ...
    m = Py_InitModule3("ecjson", ecjson_methods, module_doc);

    /* Add some symbolic constants to the module */
    if (JSON_Error == NULL) {
        JSON_Error = PyErr_NewException("ecjson.Error", NULL, NULL);
        if (JSON_Error == NULL)
            return;
    ...
    if (JSON_EncodeError == NULL) {
        JSON_EncodeError = PyErr_NewException("ecjson.EncodeError",
                                              JSON_Error, NULL);
        if (JSON_EncodeError == NULL)
            return;
    ...
    if (JSON_DecodeError == NULL) {
        JSON_DecodeError = PyErr_NewException("ecjson.DecodeError",
                                              JSON_Error, NULL);
        if (JSON_DecodeError == NULL)
            return;
